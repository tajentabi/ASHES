...
def main():
    ...
    try:
        ...
        np.savez(OUTFILE, freq_Hz=f_abs.astype(np.float64), psd_dB=psd_db.astype(np.float32),
                 center_Hz=CENTER_HZ, fs_Hz=SAMP_RATE, frames=good, window="hann",
                 dc_notch=APPLY_DC_NOTCH, samples_per_frame=SAMPLES_PER_FRAME)
